...
export class RecommendationsComponent implements OnInit, BlockableUI 
{
  ...
  savedQuantity: number = null ;                           //Default saved stock quantity
      ...
      this.barGraphGradientsNumbersOptions = 
      {
          ...
          scales: 
          {
            ...
            [{
              ...
              {
                  stepSize: 5,
                  min : this.minimumSell  ,
                  max : this.maximumSell  ,
                  maxTicksLimit : 7,
                  letterSpacing: "0.2px",
                  fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen, Ubuntu, Cantarell, 'Open Sans', 'Helvetica Neue', sans-serif" ,
                  fontColor: "black",
  ...
  findMinimum(tempStocks: StocksRecommended[])
  {
    ...
    if(this.round(Math.round(minimum)) - 10 >= 0 )
    {
       minimumParameter = this.round(Math.round(minimum)) - 10;
    }
    else
    {
